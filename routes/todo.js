...
router.use(async (req, res, next) => {
    try{
        if (!req.session.user){
            return res.status(401).json({ error: "Unauthorized: Please log in to access tasks." });
        }
...
router.get("/", async (req, res) => {
    try{
        ...
        return res.status(500).render("error", { error: "An error occurred while loading the to-do list." });
    }
});
...
router.put("/updateTask/:id", async (req, res) => {
    try{
        ...
        if(!header || !text){
            return res.status(400).json({ error: "Header and text are required to update a task." });
        }

        const updatedTask = await updateTask(userId, taskId, { header: header.trim(), text: text.trim() });
